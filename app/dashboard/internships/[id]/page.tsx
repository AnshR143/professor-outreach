...
export default async function InternshipDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: contact } = await supabase.from("internship_contacts").select("*").eq("id", id).eq("user_id", user.id).single()
  if (!contact) redirect("/dashboard/internships")
  const { data: emails } = await supabase.from("internship_emails").select("*").eq("contact_id", id).order("created_at", { ascending: false })
