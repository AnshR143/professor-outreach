...
export default function TemplatesClient({ templates: initial, userId, userName }: Props) {
  ...
  const TemplateCard = ({ t }: { t: Template }) => (
    <div onClick={() => setSelected(t)}
      style={{ background: selected?.id === t.id ? "#fff7ed" : "#fff", borderRadius: 10, border: `1px solid ${selected?.id === t.id ? "#fed7aa" : "#e2e8f0"}`, padding: 16, cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <div style={{ marginTop: 8, padding: "6px 10px", background: selected?.id === t.id ? "#fff7ed" : "#f8f9fb", borderRadius: 6, fontSize: 11, color: "#64748b", textAlign: "center", border: "1px solid #e2e8f0" }}>
          ...
          <div style={{ padding: "16px 20px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button onClick={() => setShowCreate(true)}
                style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", background: "#f97316", color: "#fff", border: "none", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                + Create Template
              </button>
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button onClick={saveTemplate} disabled={saving || !newTemplate.name}
                  style={{ flex: 2, padding: 10, background: saving || !newTemplate.name ? "#fed7aa" : "#f97316", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer" }}>
