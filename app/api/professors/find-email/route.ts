...
export async function POST(req: Request) {
  ...
  const { researcherName, university } = await req.json()
  ...
  const prompt = `Find the most likely professional email address for professor "${researcherName}" at "${university}".
Universities use patterns like first.last@school.edu, last.first@school.edu, or initials@school.edu.
Think about the specific university's email convention if you know it.
If you are uncertain, provide the most plausible guess.
Return ONLY the email address (e.g. j.smith@harvard.edu). No other text.`
