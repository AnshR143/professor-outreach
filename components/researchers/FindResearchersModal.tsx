...
export default function FindResearchersModal({ onClose, initialKeyword = "" }: Props) {
  ...
  const [step, setStep] = useState<"config" | "loading" | "seeding" | "done">("config")
  ...
  async function handleLoadPreloaded() {
    ...
    setError("")
    try {
      const res = await fetch("/api/professors/seed-preloaded", { method: "POST" })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "Seed failed")
      setSeedResult({ inserted: data.inserted, skipped: data.skipped || 0 })
      setStep("done")
      router.refresh()
    } catch (e: any) {
      setError(e.message || "Failed to load professors")
      setStep("config")
    }
  }
        ...
        <div style={{ padding: "24px" }}>
          {step === "config" && (
            <div>
              ...
              {/* Quick Load Banner */}
              <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "14px 16px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#15803d", marginBottom: 2 }}>Load 250+ Curated Professors Instantly</div>
                  <div style={{ fontSize: 12, color: "#166534" }}>Pre-vetted researchers from top universities — added instantly with resume-based match scores.</div>
                </div>
                <button
                  onClick={handleLoadPreloaded}
                  style={{ flexShrink: 0, padding: "9px 18px", background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                >
                  Load Now
                </button>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 12, color: "#94a3b8", fontWeight: 500 }}>or filter by field</span>
                <div style={{ flex: 1, height: 1, background: "#e2e8f0" }} />
              </div>

              {/* Fields — primary input */}
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 10 }}>
                  Filter by university <span style={{ color: "#94a3b8", fontWeight: 400 }}>(optional)</span>
                </label>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8, maxHeight: 100, overflowY: "auto" }}>
              ...
              <div style={{ marginBottom: 24 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 8 }}>
                  How many? <span style={{ color: "#f97316", fontWeight: 700 }}>{form.count}</span>
                </label>
              ...
              <button onClick={handleFind}
                style={{ width: "100%", padding: "12px", background: "#f97316", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          ...
          {step === "seeding" && (
            <div style={{ textAlign: "center", padding: "52px 20px" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
                <Loader9 color="#16a34a" size="lg" />
              </div>
              <h3 style={{ fontSize: 18, fontWeight: 600, color: "#0f172a", margin: "0 0 8px" }}>Loading Professor List...</h3>
              <p style={{ color: "#64748b", fontSize: 14, margin: 0 }}>Adding curated professors and calculating match scores from your resume.</p>
            </div>
          )}
          ...
          {step === "loading" && (
            <div style={{ textAlign: "center", padding: "48px 20px" }}>
              ...
              <div style={{ background: "#f1f5f9", borderRadius: 20, height: 6, overflow: "hidden", maxWidth: 300, margin: "0 auto" }}>
                <div style={{ height: "100%", background: "#f97316", borderRadius: 20, width: `${progress.total ? Math.max(8, (progress.found / progress.total) * 100) : 8}%`, transition: "width 0.4s ease" }} />
              </div>
          ...
          {step === "done" && (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              <h3 style={{ fontSize: 20, fontWeight: 700, color: "#0f172a", margin: "0 0 8px" }}>
                {seedResult ? `${seedResult.inserted} Professors Added` : `${progress.found} Researchers Found`}
              </h3>
              <p style={{ color: "#64748b", fontSize: 14, margin: "0 0 8px" }}>
                {seedResult
                  ? `${seedResult.inserted} new professors added${seedResult.skipped ? ` (${seedResult.skipped} already existed)` : ""}.`
                  : `${progress.found} researchers matched — ranked by field + resume fit.`}
              </p>
              <p style={{ color: "#94a3b8", fontSize: 12, margin: "0 0 24px" }}>
                Match scores reflect how closely each professor's work aligns with your selected field and resume background.
              </p>
              <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
                <button onClick={onClose}
                  style={{ padding: "10px 24px", background: "#f97316", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
                  View Researchers
                </button>
                <button onClick={() => { setStep("config"); setProgress({ found: 0, total: 0, current: "" }); setSeedResult(null) }}
